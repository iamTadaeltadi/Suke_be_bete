...
public class ChapaCallbackServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String prId = jsonResponse.getJSONObject("data").getString("last_name");

            if (!prId.matches("\\d+")) {
                // Product ID is not a digit, split by commas, convert to integers, and process each
                String[] nonDigitProductIds = prId.split(",");
                for (String nonDigitProductId : nonDigitProductIds) {
                    if (!nonDigitProductId.isEmpty()) {
                        Integer productId = Integer.parseInt(nonDigitProductId.trim());
                ...
                Integer productId = Integer.parseInt(prId);
                int status = response.getStatus();
                Integer quantity = Integer.parseInt(jsonResponse.getJSONObject("data").getJSONObject("customization").getString("title"));
                Integer userId = Integer.parseInt(jsonResponse.getJSONObject("data").getJSONObject("customization").getString("description"));
                System.out.println(status);
